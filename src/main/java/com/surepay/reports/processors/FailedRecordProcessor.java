...
/*
 * Process a file based on received reader, reporter and validation rules
 * after processing all the record it generates the output file with failed records
 */
    ...
    for (int i = 0; i < allRecords.size(); i++) {
      ...
      for (int j = 0; j < validatorsList.size() ; j++) {
        ...
        if(!rule.isValid(currentRecord)) {
          logger.log(Level.INFO, currentRecord.getReference()+ " reference failed validation" + rule.getClass());
